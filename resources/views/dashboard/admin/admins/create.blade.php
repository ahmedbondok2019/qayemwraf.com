        ...
        <div class="content-header row">
            <div class="content-header-left col-md-9 col-12 mb-2">
                <div class="row breadcrumbs-top">
                    <div class="col-12">
                        <h2 class="content-header-title float-left mb-0">{{ trans_db('dashboard.Add New Admin') }}</h2>
                    </div>
                </div>
            </div>
        </div>
                        ...
                        <div class="row">
                            ...
                            <div class="col-md-6 form-group">
                                ...
                                <select name="permission_group" id="permission_group" class="form-control @error('permission_group') is-invalid @enderror" required>
                                    <option value="">{{ trans_db('dashboard.Select Role') }}</option>
                                    @foreach($roles as $role)
                                    <option value="{{ $role->id }}" {{ old('permission_group') == $role->id ? 'selected' : '' }}>{{ $role->name }}</option>
                                    @endforeach
                                </select>
