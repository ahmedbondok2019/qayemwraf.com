                        ...
                        <div class="col-12">
                            ...
                            <div class="breadcrumb-wrapper">
                                <ol class="breadcrumb">
                                    ...
                                    <li class="breadcrumb-item active">{{ trans_db('dashboard.Governorates') }}
                                    </li>
                                </ol>
                            </div>
                        </div>
                        ...
                        <div class="col-12">
                            <div class="card">
                                <div class="card-header border-bottom">
                                    <h4 class="card-title">{{ trans_db('dashboard.Governorates List') }}</h4>
                                </div>
                                <div class="card-body mt-2">
                                    <table class="datatables-basic table" id="governorates-table">
                                        <thead>
                                            <tr>
                                                <th>#</th>
                                                <th>{{ trans_db('dashboard.Name') }} ({{ app()->getLocale() }})</th>
                                                <th>{{ trans_db('dashboard.Country') }}</th>
                                        ...
                                        <tbody>
                                            @foreach($governorates as $governorate)
                                                <tr>
                                                    ...
                                                    <td>{{ $governorate->translation->name ?? $governorate->translations->first()->name ?? 'N/A' }}</td>
                                                    <td>{{ $governorate->country->translation->name ?? $governorate->country->translations->first()->name ?? 'N/A' }}</td>
                                                    <td>
                                                    ...
                                                    <td>
                                                        <div class="dropdown">
                                                            ...
                                                            <div class="dropdown-menu">
                                                                <a class="dropdown-item" href="{{ route('admin.governorates.edit', $governorate->id) }}">
                                                                    <i data-feather="edit-2" class="mr-50"></i>
                                                                    <span>{{ trans_db('dashboard.Edit') }}</span>
                                                                </a>
                                                                <form action="{{ route('admin.governorates.destroy', $governorate->id) }}" method="POST" onsubmit="return confirm('{{ trans_db('dashboard.Are you sure?') }}');">
                                                                    @csrf
                                                                    @method('DELETE')
...
@section('script')
    <script>
        $(document).ready(function() {
            if ($.fn.DataTable.isDataTable('#governorates-table')) {
                $('#governorates-table').DataTable().destroy();
            }
