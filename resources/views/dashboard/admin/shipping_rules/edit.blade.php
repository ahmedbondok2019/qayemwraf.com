                    ...
                    <div class="row">
                        <div class="col-12">
                            <div class="card">
                                ...
                                <div class="card-body">
                                    <form class="form" action="{{ route('admin.shipping_rules.update', $rule->id) }}" method="POST">
                                        @csrf
                                        @method('PUT')
                                        <div class="row">
                                                    ...
                                                    <div class="form-group">
                                                        ...
                                                        <div class="input-group">
                                                            ...
                                                            <input type="text" id="name_{{ $localeCode }}"
                                                                ...
                                                                value="{{ old('name_' . $localeCode, $rule->translations->where('locale', $localeCode)->first()->name ?? '') }}" required placeholder="{{ trans_db('dashboard.Name') }} ({{ $properties['native'] }})" />
                                                        </div>
                                                    </div>
                                                </div>
                                            @endforeach
                                                ...
                                                <div class="form-group">
                                                    ...
                                                    <div class="input-group">
                                                        ...
                                                        <select name="country_id" id="country_id" class="form-control" required>
                                                            <option value="" disabled>{{ trans_db('dashboard.Choose Country') }}</option>
                                                            @foreach($countries as $country)
                                                                <option value="{{ $country->id }}" {{ $rule->country_id == $country->id ? 'selected' : '' }}>{{ $country->name }}</option>
                                                            @endforeach
                                                        </select>
                                                    </div>
                                                </div>
                                                ...
                                                <div class="form-group">
                                                    ...
                                                    <div class="custom-control custom-switch custom-switch-success">
                                                        <input type="checkbox" class="custom-control-input" id="is_active"
                                                            name="is_active" {{ $rule->is_active ? 'checked' : '' }} />
                                                        ...
                                                        <tbody id="governorates-body">
                                                            @foreach($governorates as $gov)
                                                                <tr>
                                                                    ...
                                                                    <td>{{ $gov->name }}</td>
                                                                    <td>
                                                                        <input type="number" step="0.01" name="rates[{{ $gov->id }}]" class="form-control" placeholder="{{ trans_db('dashboard.Value') }}" 
                                                                        value="{{ $rates[$gov->id] ?? 0 }}">
                                                                    </td>
                                                                </tr>
                                                            @endforeach
                                                        </tbody>
...
<script>
    $('#country_id').change(function() {
        ...
        // Warn user that changing country will reset rates? Maybe handled backend or UI warning
        if(countryId) {
            $.ajax({
