                ...
                <section class="app-user-edit">
                    <div class="card">
                        <div class="card-header">
                            <h3>{{ trans_db('dashboard.Create New Currency') }}</h3>
                        </div>
                        <div class="card-body">
                            <form action="{{ route('admin.currencies.store') }}" method="POST">
                                ...
                                <div class="row">
                                    ...
                                    <div class="col-md-6 mb-1">
                                        ...
                                        <input type="number" step="0.00000001" name="exchange_rate" class="form-control" value="{{ old('exchange_rate') }}" required>
                                    </div>

                                    <div class="col-md-6 mb-1">
                                        <label class="form-label">{{ trans_db('dashboard.Status') }}</label>
                                        <select name="status" class="form-control">
                                            <option value="1" {{ old('status') == 1 ? 'selected' : '' }}>{{ trans_db('dashboard.active') }}</option>
                                            <option value="0" {{ old('status') == 0 ? 'selected' : '' }}>{{ trans_db('dashboard.inactive') }}</option>
                                        </select>
                                    </div>
                                    ...
                                    <div class="col-md-6 mb-1">
                                        ...
                                        <select name="is_default" class="form-control">
                                            <option value="0" {{ old('is_default') == 0 ? 'selected' : '' }}>{{ trans_db('dashboard.no') }}</option>
                                            <option value="1" {{ old('is_default') == 1 ? 'selected' : '' }}>{{ trans_db('dashboard.yes') }}</option>
                                        </select>
                                    </div>
                                                ...
                                                <div class="tab-pane fade {{ $loop->first ? 'show active' : '' }}" id="{{ $locale }}" role="tabpanel" aria-labelledby="{{ $locale }}-tab">
                                                    <div class="row">
                                                        ...
                                                        <div class="col-md-6 mb-1">
                                                            ...
                                                            <input type="text" name="{{ $locale }}[symbol]" class="form-control" value="{{ old($locale . '.symbol') }}" required>
                                                        </div>
                                                    </div>
                                                </div>
                                            @endforeach
