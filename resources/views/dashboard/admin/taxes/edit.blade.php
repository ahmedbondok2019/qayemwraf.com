                        ...
                        <div class="card-body">
                            <div class="tab-content">
                                ...
                                <div class="tab-pane active" id="account" aria-labelledby="account-tab" role="tabpanel">
                                    ...
                                    <form action="{{ \LaravelLocalization::localizeUrl('admin-2023/tax/update') }}" method="post" enctype="multipart/form-data" role="form">
                                        @csrf
                                                ...
                                                <div class="row">
                                                    ...
                                                    <div class="col-lg-3">
                                                        <div class="form-group {{ $errors->has('status') ? 'has-error' : '' }}">
                                                            ...
                                                            <select class="form-control form-select-lg" name="status" id="select2-basic" wire:model="status">
                                                                <option value="0" {{ $details->status == 0 ? "selected" : "" }}>{{ trans_db("dashboard.NotActive") }}</option>
                                                    ...
                                                    {{-- <div class="col-lg-3">
                                                        <div class="form-group {{ $errors->has('payment_method') ? 'has-error' : '' }}">
                                                            ...
                                                            <select class="form-control form-select-lg" name="payment_method" id="select2-basic" wire:model="status">
                                                                <option value="0" {{ $details->payment_method == 0 ? "selected" : "" }}>{{ trans_db("dashboard.Choose") }}</option>
                                                                <option value="1" {{ $details->payment_method == 0 ? "selected" : "" }}>{{ trans_db("dashboard.Cash") }}</option>
                                                                <option value="2" {{ $details->payment_method == 1 ? "selected" : "" }}>{{ trans_db("website.Fawry") }}</option>
                                                                <option value="3" {{ $details->payment_method == 1 ? "selected" : "" }}>{{ trans_db("dashboard.Paytabs") }}</option>
                                                            </select>
                                                            @error('status') <span class="text-danger error">{{ $message }}</span> @enderror
                                                        </div>
                                                    </div> --}}
                                                        ...
                                                        <div class="col-lg-12">
                                                            <div class="mb-1">
                                                                <select class="select2-size-lg form-select" multiple="multiple" id="selected_payment_methods" name="selected_payment_methods">
                                                                    @foreach (array_filter(explode(',' , $details->payment_method), fn($value) => !is_null($value) && $value !== '') as $payment_methods)
                                                                        @if($payment_methods == 0) <option value="0" selected>{{ trans_db("dashboard.Choose") }}</option> @endif
                                                                        @if($payment_methods == 1) <option value="1" selected>{{ trans_db("dashboard.Cash") }}</option> @endif
                                                                        @if($payment_methods == 2) <option value="2" selected>{{ trans_db("website.Fawry") }}</option> @endif
                                                                        @if($payment_methods == 3) <option value="3" selected>{{ trans_db("dashboard.Paytabs") }}</option> @endif
                                                                    @endforeach
                                                                </select>
                                                                <input type="hidden" name="payment_methods" value="{{ $details->payment_methods }}">
                                                            </div>
                                                        </div>
                                                    </div>
                                                </div>
                                            ...
                                            <div class="card-content collapse show" style="">
                                                <div class="card-body">
                                                    <div class="row">
                                                        <div class="col-lg-12">
                                                            <div class="mb-1">
                                                                <select class="select2-size-lg form-select" multiple="multiple" id="selected_product_categories" name="selected_product_categories">
                                                                    @foreach (array_filter(explode(',' , $details->product_categories), fn($value) => !is_null($value) && $value !== '') as $product_categories)
                                                                        @foreach ($categories as $category)
                                                                            @if ($category->id == $product_categories)
                                                                                <option value="{{ $category->id }}" selected data-cat="{{ $product_categories }}">{{ $category->CategoryTranslation->title }}</option>
                                                                            @endif
                                                                        @endforeach
                                                                    @endforeach
                                                                </select>
                                                                <input type="hidden" name="product_categories" value="{{ $details->product_categories }}">
                                                            </div>
                                                        </div>
                                                    </div>
                                                </div>
