                                            ...
                                            <div class="col-12">
                                                <ul class="nav nav-tabs" role="tablist">
                                                    @foreach(LaravelLocalization::getSupportedLocales() as $localeCode => $properties)
                                                        @php
                                                            $translation = $category->translations->where('locale', $localeCode)->first();
                                                        @endphp
                                                        <li class="nav-item">
                                                            ...
                                                            <div class="row">
                                                                ...
                                                                <div class="col-12">
                                                                    <div class="form-group">
                                                                        ...
                                                                        <textarea id="description_{{ $localeCode }}" class="form-control" name="description_{{ $localeCode }}">{{ old('description_' . $localeCode, $translation->description ?? '') }}</textarea>
                                                                    </div>
                                                                </div>
                                                                <div class="col-12">
                                                                    <div class="form-group">
                                                                        <label for="meta_description_{{ $localeCode }}">{{ trans_db('dashboard.Meta Description') }} ({{ $properties['native'] }})</label>
                                                                        <textarea id="meta_description_{{ $localeCode }}" class="form-control" name="meta_description_{{ $localeCode }}">{{ old('meta_description_' . $localeCode, $translation->meta_description ?? '') }}</textarea>
                                                                    </div>
                                                                </div>
                                                                <div class="col-12">
                                                                    <div class="form-group">
                                                    ...
                                                    <select class="form-control" id="parent_id" name="parent_id">
                                                        ...
                                                        @foreach($categories as $cat)
                                                            <option value="{{ $cat->id }}" {{ $category->parent_id == $cat->id ? 'selected' : '' }}>
                                                                {{ $cat->translation->title ?? $cat->translations->first()->title ?? 'N/A' }}
                                                            </option>
                                                        @endforeach
                                                    </select>
                                                </div>
                                            ...
                                            <div class="col-md-6 col-12">
                                                <div class="form-group">
                                                    ...
                                                    <div class="custom-control custom-switch custom-switch-success">
                                                        <input type="checkbox" class="custom-control-input" id="is_active" name="is_active" {{ $category->is_active ? 'checked' : '' }}>
