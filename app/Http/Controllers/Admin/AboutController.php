<?php

namespace App\Http\Controllers\Admin;

use App\Http\Controllers\helper\HelperController;
use App\Http\Requests\About\CreateAboutRequest;
use App\Http\Requests\About\CreateAboutTransRequest;
use App\Http\Requests\About\UpdateAboutRequest;
use App\Models\About;
use App\Models\AboutImage;
use App\Models\AboutTranslation;
use Illuminate\Http\Request;
use Illuminate\Support\Arr;
use Illuminate\Support\Carbon;
use Illuminate\Support\Facades\Session;
use Illuminate\Support\Str;
use Mcamara\LaravelLocalization\Facades\LaravelLocalization;

class AboutController extends BackendController
{
    public function index()
    {
        if (! in_array('42', Session::get('permissionData'))) {
            return redirect()->back();
        }

        $data['about'] = About::whereHas('AboutTranslation')->with('AboutImages')->first();

        return view('dashboard.admin.about.index', $data);
    }

    public function addTrans(Request $request)
    {
        if (! in_array('42', Session::get('permissionData'))) {
            return redirect()->back();
        }
        $data['title'] = trans_db('dashboard.CreateNewSliderTrans');
        $data['id'] = $request->about_id;

        return view('dashboard.admin.about.index', $data);
    }

    public function edit(Request $request)
    {
        if (! in_array('42', Session::get('permissionData'))) {
            return redirect()->back();
        }
        $data['Details'] = About::where('id', $request->id)
            ->with('AboutTranslation')->where('lang_id', app()->getLocale())->firstOrFail();
        if ($data['Details']->AboutTranslation == null) {
            return redirect('/admin-2023/about/addTrans/'.$request->id);
        }
        $data['id'] = $request->id;

        return view('dashboard.admin.about.index', $data);
    }

    public function store(CreateAboutRequest $request)
    {
        if (! in_array('42', Session::get('permissionData'))) {
            return redirect()->back();
        }
        $Createabout = About::create([
            'lang_id' => app()->getLocale(),
            'view_index' => $request->view_index == null ? 0 : $request->view_index,
        ]);

        if (isset($request->image)) {
            $data = self::imageUpload($request, $request->image, '1920', '1280');
            $image_name = $data['image'];
        }

        $input = Arr::except($request->all(), ['_token', 'image', 'cropped_image']);
        $inputs = array_merge($input, [
            'slug' => $request->slug,
            'video_link' => $request->video_link,
            'description' => $request->description,
            'image' => $image_name,
            'history' => $request->history,
            'vision' => $request->vision,
            'mission' => $request->mission,
            'about_id' => $Createabout->id,
            'lang_id' => app()->getLocale(),
            'view_index' => $request->view_index == null ? 0 : $request->view_index,
        ]);
        $about = AboutTranslation::create($inputs);
        if (! empty($request->images)) {
            self::images(
                $request->images,
                $request->slug,
                $Createabout->id,
                $about->id,
                app()->getLocale()
            );
        }

        if ($data['type'] == 'success') {
            alert()->success($data['body'], $data['title']);

            return redirect('/admin-2023/about/all');
        } else {
            alert()->error($data['body'], $data['title']);

            return redirect('/admin-2023/about/add');
        }
    }

    public function addAboutTrans(CreateAboutTransRequest $request)
    {
        if (! in_array('42', Session::get('permissionData'))) {
            return redirect()->back();
        }
        if (isset($request->image)) {
            $data = self::imageUpload($request, $request->image, '1170', '453');
            $image_name = $data['image'];
        }

        // / check if not upload any image get the first image.
        if (empty($image_name)) {
            $firstTrans = AboutTranslation::where('about_id', $request->about_id)->first();
            $image_name = $firstTrans->image;
        }

        // // create parent category for all trans.
        About::find($request->about_id);
        $input = Arr::except($request->all(), ['_token', 'image', 'cropped_image']);
        $inputs = array_merge($input, [
            'slug' => $request->slug,
            'video_link' => $request->video_link,
            'description' => $request->description,
            'image' => $image_name,
            'history' => $request->history,
            'vision' => $request->vision,
            'mission' => $request->mission,
            'about_id' => $request->about_id,
            'lang_id' => app()->getLocale(),
            'view_index' => $request->view_index == null ? 0 : $request->view_index,
        ]);
        $test = AboutTranslation::where('about_id', $request->about_id)
            ->where('lang_id', app()->getLocale())->first();
        if (isset($test)) {
            alert()->error(trans_db('dashboard.Duplicate_TitleOrLanguage'), trans_db('dashboard.attention'));

            return redirect('/admin-2023/about/addTrans/'.$request->about_id);
        }

        if (AboutTranslation::create($inputs)) {
            alert()->success(trans_db('dashboard.saved'), trans_db('dashboard.congratulation'));

            return redirect('/admin-2023/about/all');
        } else {
            alert()->error(trans_db('dashboard.notsaved'), trans_db('dashboard.attention'));

            return redirect('/admin-2023/about/addTrans/'.$request->about_id);
        }
    }

    public function update(UpdateAboutRequest $request)
    {
        if (! in_array('42', Session::get('permissionData'))) {
            return redirect()->back();
        }
        $Createabout = About::whereHas('AboutTranslation')->whereHas('AboutImages')->first();
        if (empty($Createabout)) {
            $Createabout = About::create([
                'lang_id' => app()->getLocale(),
                'view_index' => $request->view_index == null ? 0 : $request->view_index,
            ]);
        }

        if (isset($request->image)) {
            $data = self::imageUpload($request, $request->image, '1170', '453');
            $image_name = $data['image'];
        }

        $aboutTrans = AboutTranslation::where('about_id', $request->id)
            ->where('lang_id', app()->getLocale())->first();

        if (! empty($request->images)) {
            self::images(
                $request->images,
                $request->slug,
                $Createabout->id,
                $Createabout->id,
                app()->getLocale()
            );
        }

        if (empty($aboutTrans)) {
            $aboutTrans = AboutTranslation::create([
                'slug' => $request->slug,
                'video_link' => $request->video_link,
                'description' => $request->description,
                'image' => isset($image_name) ? $image_name : null,
                'history' => $request->history,
                'vision' => $request->vision,
                'mission' => $request->mission,
                'about_id' => $Createabout->id,
                'lang_id' => app()->getLocale(),
                // 'view_index' => $request->view_index == null ? 0 : $request->view_index
            ]);

            alert()->success(trans_db('dashboard.saved'), trans_db('dashboard.congratulation'));

            return redirect(LaravelLocalization::localizeUrl('admin-2023/about/all'));
        } else {
            if ($aboutTrans->update([
                'slug' => $request->slug,
                'video_link' => $request->video_link,
                'description' => $request->description,
                'image' => isset($image_name) ? $image_name : $aboutTrans->image,
                'history' => $request->history,
                'vision' => $request->vision,
                'mission' => $request->mission,
                'about_id' => $Createabout->id,
                'lang_id' => app()->getLocale(),
            ])) {
                alert()->success(trans_db('dashboard.saved'), trans_db('dashboard.congratulation'));

                return redirect(LaravelLocalization::localizeUrl('admin-2023/about/all'));
            } else {
                alert()->error(trans_db('dashboard.notsaved'), trans_db('dashboard.attention'));

                return redirect(LaravelLocalization::localizeUrl('admin-2023/about/all'));
            }
        }
    }

    public static function images($images, $About_name, $AboutId, $transId, $lang_id)
    {
        if (! empty($images)) {
            foreach ($images as $image) {
                $imageSlug = HelperController::make_slug($About_name.Str::random('8').'_'.str_replace(' ', '', Carbon::today()));
                $image_name = str_replace(' ', '', $imageSlug).'.png';

                $relativePath = self::UploadImagesAbout($image, $image_name, 'about', '382', '476');

                AboutImage::create([
                    'image' => $relativePath,
                    'about_id' => $AboutId,
                    'translation_id' => $transId,
                    'lang_id' => $lang_id,
                ]);
            }
        }
    }

    public function delete(Request $request)
    {
        $data = AboutTranslation::where('about_id', $request->id)->first();
        $parentCheck = AboutTranslation::where('about_id', $request->id)->count();
        if ($parentCheck < 2) {
            About::where('id', $request->id)->delete();
        }
        if (! empty($data->image)) {
            $oldPath = str_replace('storage/', '', $data->image);
            if (\Illuminate\Support\Facades\Storage::disk('public')->exists($oldPath)) {
                \Illuminate\Support\Facades\Storage::disk('public')->delete($oldPath);
            } elseif (file_exists(public_path('website/images/about/' . $data->image))) {
                unlink(public_path('website/images/about/' . $data->image));
            }
        }

        $data->delete();

        alert()->success(trans_db('dashboard.Deleted Successfully..'), trans_db('dashboard.congratulation'));

        return redirect('admin-2023/about/all');
    }

    public function AboutsOrderUp(Request $request)
    {
        $About = About::find($request->id);
        $view_index = $About->view_index;

        if ($view_index > 0) {
            $Abouts = About::where('view_index', $view_index - 1)->get();
            foreach ($Abouts as $Aboutss) {
                $Aboutss->update(['view_index' => $view_index]);
            }
            $About->view_index == null ? $new_index = 1 : $new_index = $view_index - 1;
            $About->view_index = $new_index;
            $About->save();
        }

        return redirect('/admin-2023/about/all');
    }

    public function AboutsOrderDown(Request $request)
    {
        $About = About::find($request->id);
        $count = About::count();
        $view_index = $About->view_index;

        if ($view_index < $count) {
            $Abouts = About::where('view_index', $view_index + 1)->get();
            foreach ($Abouts as $Cour) {
                $Cour->update(['view_index' => $view_index]);
            }

            $About->view_index = $view_index + 1;
            $About->save();
        }

        return redirect('/admin-2023/about/all');
    }

    public static function imageUpload(Request $request, $image, $width, $height, $oldImage = null)
    {
        $image_nam = HelperController::make_slug(
            Str::random('8').'_'.str_replace(' ', '', Carbon::today())
        );
        $image_name = str_replace(' ', '', $image_nam).'.png';

        if ($request->cropped_image != null && file_exists(public_path($request->cropped_image))) {
            self::UploadImagesAbout(
                public_path($request->cropped_image), $image_name, 'about', $width, $height
            );
            unlink(public_path($request->cropped_image));

            return ['image' => $image_name, 'body' => trans_db('dashboard.saved'), 'title' => trans_db('dashboard.congratulation'), 'type' => 'success'];
        }

        if ($image) {
            if (isset($oldImage) && $oldImage != null) {
                $oldPath = str_replace('storage/', '', $oldImage);
                if (\Illuminate\Support\Facades\Storage::disk('public')->exists($oldPath)) {
                    \Illuminate\Support\Facades\Storage::disk('public')->delete($oldPath);
                } elseif (file_exists(public_path('website/images/about/'.$oldImage))) {
                    unlink(public_path('website/images/about/'.$oldImage));
                }
            }
            $relativePath = self::UploadImagesAbout($image, $image_name, 'about', $width, $height);

            return ['image' => $relativePath, 'body' => trans_db('dashboard.saved'), 'title' => trans_db('dashboard.congratulation'), 'type' => 'success'];
        } else {
            return ['image' => null, 'body' => trans_db('dashboard.InValidImage'), 'title' => trans_db('dashboard.attention'), 'type' => 'error'];
        }
    }

    public static function UploadImagesAbout($image, $name, $folder, $width = null, $height = null)
    {
        $path = 'website' . DIRECTORY_SEPARATOR . 'images' . DIRECTORY_SEPARATOR . $folder;
        $fullStoragePath = storage_path('app/public' . DIRECTORY_SEPARATOR . $path);
        $destination = $fullStoragePath . DIRECTORY_SEPARATOR . $name;

        HelperController::upload_images($fullStoragePath, $destination, $image, $width, $height);
        
        return 'storage/website/images/' . $folder . '/' . $name;
    }

    public function cropAbout(Request $request)
    {
        $data = $request->image;
        // //data:image/png;base64,
        $image_array_1 = explode(';', $data);

        // base64,
        $image_array_2 = explode(',', $image_array_1[1]);

        // 
        $data = base64_decode($image_array_2[1]);
        //        return $data;
        $image_name = HelperController::make_slug(Carbon::now());

        $path = public_path('website'.DIRECTORY_SEPARATOR.'images'.DIRECTORY_SEPARATOR.'temporary');
        $destination = public_path('website'.DIRECTORY_SEPARATOR.'images'.DIRECTORY_SEPARATOR.'temporary'.DIRECTORY_SEPARATOR.$image_name.'.png');
        HelperController::upload_images($path, $destination, $data, '1170', '453');

        return '/website'.DIRECTORY_SEPARATOR.'images'.DIRECTORY_SEPARATOR.'temporary'.DIRECTORY_SEPARATOR.$image_name.'.png';
    }

    public function change_status(Request $request)
    {
        $user = About::find($request->about_id);
        $user->update([
            'status' => $request->status,
        ]);

        return response()->json(['data' => 'success']);

    }

    public function delete_image(Request $request)
    {
        $data = AboutImage::find($request->id);
        if ($data->image) {
            $oldPath = str_replace('storage/', '', $data->image);
            if (\Illuminate\Support\Facades\Storage::disk('public')->exists($oldPath)) {
                \Illuminate\Support\Facades\Storage::disk('public')->delete($oldPath);
            } elseif (file_exists(public_path('website/images/about/' . $data->image))) {
                unlink(public_path('website/images/about/' . $data->image));
            }
        }
        $data->delete();

        alert()->success(trans_db('dashboard.Deleted Successfully..'), trans_db('dashboard.congratulation'));

        return redirect(LaravelLocalization::localizeUrl('admin-2023/about/all'));
    }
}
