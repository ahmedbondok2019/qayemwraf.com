...
class BrandsController extends BackendController
{
    public function index()
    {
        ...
        return view('dashboard.admin.brands.index', $data);
    }
    ...
    public function addTrans(Request $request)
    {
        ...
        $data['Details'] = Brand::where('id', $request->brand_id)->first();
        if ($data['Details'] == null) {
            return redirect('/admin-2023/brands/all');
        }
        $data['id'] = $request->brand_id;

        return view('dashboard.admin.brands.trans', $data);
    }

    public function edit(Request $request)
    {
        if (! in_array('35', Session::get('permissionData'))) {
            return redirect()->back();
        }
        $data['details'] = Brand::where('id', $request->id)->whereHas('BrandTranslations')->first();
        if ($data['details'] == null) {
            return redirect('/admin-2023/brands/addTrans/'.$request->id);
        }

        return view('dashboard.admin.brands.edit', $data);
    }
    ...
    public function store(CreateBrandRequest $request)
    {
        ...
        $data = self::storeBrandTranslations($request);
        if ($data == true) {
            ...
            return redirect('/admin-2023/brands/create');
        }
    }

    public function addBrandTrans(CreateBrandRequest $request)
    {
        $test = BrandTranslation::where('brand_id', $request->brand_id)
            ->where('lang_id', app()->getLocale())->first();
        if (isset($test)) {
            alert()->error(trans_db('dashboard.Duplicate_TitleOrLanguage'), trans_db('dashboard.attention'));

            return redirect('/admin-2023/brands/addTrans/'.$request->brand_id);
        }

        $data = self::storeBrandTranslations($request, 'trans');
        if ($data == true) {
            alert()->success(trans_db('dashboard.saved'), trans_db('dashboard.congratulation'));
    ...
    public static function storeBrandTranslations(Request $request, $type = null)
    {
        if ($type == null) {
        ...
        if ($type != null) {
            $CreateBrand = Brand::findOrFail($request->brand_id);
        }
        ...
        BrandTranslation::create([
            ...
            'image' => $image_name,
            'brand_id' => isset($CreateBrand) ? $CreateBrand->id : $request->brand_id,
            'lang_id' => app()->getLocale(),
        ]);
    ...
    public function update(UpdateBrandRequest $request)
    {
        ...
        if (is_numeric($request->brand_id)) {
            $data = self::updateBrandTranslations($request);
            ...
            return redirect('/admin-2023/brands/edit/'.$request->brand_id);
    ...
    public static function updateBrandTranslations(Request $request)
    {
