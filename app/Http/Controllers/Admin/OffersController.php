<?php

namespace App\Http\Controllers\Admin;

use App\Http\Controllers\helper\HelperController;
use App\Http\Requests\offers\CreateOfferRequest;
use App\Http\Requests\offers\UpdateOfferRequest;
use App\Models\Category;
use App\Models\Offer;
use App\Models\OfferTranslation;
use Illuminate\Http\Request;
use Illuminate\Support\Carbon;
use Illuminate\Support\Facades\Session;
use Illuminate\Support\Str;

class OffersController extends BackendController
{
    public function index()
    {
        if (! in_array('69', Session::get('permissionData'))) {
            return redirect()->back();
        }
        $data['offers'] = Offer::all();
        $data['details'] = Offer::where('static', 1)->whereHas('offer_translations')->first();
        if ($data['details'] == null) {
            $CreateOffer = Offer::create([
                'static' => 1,
            ]);
            OfferTranslation::create([
                'title' => app()->getLocale(),
                'offer_id' => $CreateOffer->id,
                'lang_id' => app()->getLocale(),
            ]);
            $data['details'] = Offer::where('static', 1)->whereHas('offer_translations')->first();
        }
        $data['categories'] = Category::whereHas('CategoryTranslation')->orderByDesc('id')->get();

        return view('dashboard.admin.offers.index', $data);
    }

    public function create()
    {
        if (! in_array('70', Session::get('permissionData'))) {
            return redirect()->back();
        }

        return view('dashboard.admin.offers.create');
    }

    public function addTrans(Request $request)
    {
        if (! in_array('70', Session::get('permissionData'))) {
            return redirect()->back();
        }
        $data['Details'] = Offer::where('id', $request->offer_id)->first();
        if ($data['Details'] == null) {
            return redirect('/admin-2023/offers/all');
        }
        $data['id'] = $request->offer_id;
        $data['categories'] = Category::whereHas('CategoryTranslation')->orderByDesc('id')->get();

        return view('dashboard.admin.offers.trans', $data);
    }

    public function edit(Request $request)
    {
        if (! in_array('71', Session::get('permissionData'))) {
            return redirect()->back();
        }
        $data['details'] = Offer::where('id', $request->id)->whereHas('offer_translations')->first();
        if ($data['details'] == null) {
            return redirect('/admin-2023/offers/addTrans/'.$request->id);
        }
        $data['categories'] = Category::whereHas('CategoryTranslation')->orderByDesc('id')->get();

        return view('dashboard.admin.offers.edit', $data);
    }

    public function store(CreateOfferRequest $request)
    {
        if (! in_array('70', Session::get('permissionData'))) {
            return redirect()->back();
        }

        $data = self::storeOfferTranslations($request);
        if ($data == true) {
            alert()->success(trans_db('dashboard.saved'), trans_db('dashboard.congratulation'));

            return redirect('/admin-2023/offers/all');
        } else {
            alert()->error(trans_db('dashboard.notsaved'), trans_db('dashboard.attention'));

            return redirect('/admin-2023/offers/create');
        }
    }

    public function addOfferTrans(CreateOfferRequest $request)
    {
        if (! in_array('70', Session::get('permissionData'))) {
            return redirect()->back();
        }

        $test = OfferTranslation::where('offer_id', $request->offer_id)
            ->where('lang_id', app()->getLocale())->first();
        if (isset($test)) {
            alert()->error(trans_db('dashboard.Duplicate_TitleOrLanguage'), trans_db('dashboard.attention'));

            return redirect('/admin-2023/offers/addTrans/'.$request->offer_id);
        }

        $data = self::storeOfferTranslations($request, 'trans');
        if ($data == true) {
            alert()->success(trans_db('dashboard.saved'), trans_db('dashboard.congratulation'));

            return redirect('/admin-2023/offers/all');
        } else {
            alert()->error(trans_db('dashboard.notsaved'), trans_db('dashboard.attention'));

            return redirect('/admin-2023/offers/addTrans/'.$request->offer_id);
        }
    }

    public static function storeOfferTranslations(Request $request, $type = null)
    {
        if ($request->link == null && $request->category == null) {
            return false;
        }

        if ($type == null) {
            $max = Offer::max('views');
            $CreateOffer = Offer::create([
                'views' => $max,
                'link' => $request->link,
            ]);
        }

        if ($type != null) {
            $CreateOffer = Offer::findOrFail($request->offer_id);
        }

        $data = self::imageUpload($request);
        $image_name = $data['image'];

        OfferTranslation::create([
            'title' => strip_tags($request->title),
            'slug' => strip_tags($request->slug),
            'category' => $request->category,
            'position' => $request->position,
            'image' => $image_name,
            'offer_id' => $CreateOffer->id,
            'lang_id' => app()->getLocale(),
        ]);

        return true;
    }

    public function update(UpdateOfferRequest $request)
    {
        if (! in_array('71', Session::get('permissionData'))) {
            return redirect()->back();
        }
        if (is_numeric($request->offer_id)) {
            $data = self::updateOfferTranslations($request);

            if ($data == true) {
                alert()->success(trans_db('dashboard.saved'), trans_db('dashboard.congratulation'));
            } else {
                alert()->error(trans_db('dashboard.notsaved'), trans_db('dashboard.attention'));
            }

            return redirect('/admin-2023/offers/all');

        } else {
            alert()->error(trans_db('dashboard.User Id Wrong', trans_db('dashboard.attention')));

            return redirect()->back();
        }
    }

    public static function updateOfferTranslations(Request $request)
    {
        // if ($request->link == null && $request->category == null) {
        //     return false;
        // }

        $Offer = Offer::where('id', $request->offer_id)->first();
        $Offer->update([
            'link' => $request->link,
        ]);

        if ($request->has('image')) {
            $data = self::imageUpload($request);
            $image_name = $data['image'];
        }

        $firstImage = OfferTranslation::where('offer_id', $request->offer_id)
            ->where('lang_id', app()->getLocale())->first();

        OfferTranslation::where('offer_id', $request->offer_id)
            ->where('lang_id', app()->getLocale())
            ->update([
                'title' => strip_tags($request->title),
                'slug' => strip_tags($request->slug),
                'category' => $request->category,
                'position' => $request->position,
                'image' => $request->has('image') ? $image_name : $firstImage->image,
                'offer_id' => $Offer->id,
                'lang_id' => app()->getLocale(),
            ]);

        return true;
    }

    public function delete(Request $request)
    {
        if (! in_array('72', Session::get('permissionData'))) {
            return redirect()->back();
        }
        Offer::where('id', $request->id)->delete();

        alert()->success(trans_db('dashboard.deleted'), trans_db('dashboard.congratulation'));

        return redirect('admin-2023/offers/all');
    }

    public static function imageUpload($request)
    {
        $image_nam = HelperController::make_slug(Str::random('8').'_'.str_replace(' ', '', Carbon::today()));
        $image_name = str_replace(' ', '', $image_nam).'.png';
        $scale = self::getImageScale($request->position);
        if ($request->cropped_image != null && file_exists(public_path($request->cropped_image))) {
            // self::UploadImagesSlider(public_path($request->cropped_image) , $image_name , 'sliders' . DIRECTORY_SEPARATOR . 'small' , '1920 ','970');

            self::UploadImagesOffer(public_path($request->cropped_image), $image_name, 'offers', $scale['width'], $scale['height']);
            unlink(public_path($request->cropped_image));

            return ['image' => $image_name, 'body' => trans_db('dashboard.saved'), 'title' => trans_db('dashboard.congratulation'), 'type' => 'success'];
        }

        if (! empty($request->image) && empty($request->cropped_image)) {
            $ex = $request->image->getClientOriginalExtension();
            if (in_array($ex, ['png', 'jpeg', 'jpg', 'JPG', 'jfif', 'webp'])) {
                self::UploadImagesOffer($request->image, $image_name, 'offers', $scale['width'] ?? '64', $scale['height'] ?? '64');

                return ['image' => $image_name, 'body' => trans_db('dashboard.saved'), 'title' => trans_db('dashboard.congratulation'), 'type' => 'success'];
            } else {
                return ['image' => $image_name, 'body' => trans_db('dashboard.notsaved'), 'title' => trans_db('dashboard.attention'), 'type' => 'error'];
            }
        } else {
            return ['image' => $image_name, 'body' => trans_db('dashboard.InValidImage'), 'title' => trans_db('dashboard.attention'), 'type' => 'error'];
        }
    }

    public static function UploadImagesOffer($image, $name, $folder, $width = null, $height = null)
    {
        $path = public_path('website'.DIRECTORY_SEPARATOR.'images'.DIRECTORY_SEPARATOR.$folder);
        $destination = public_path('website'.DIRECTORY_SEPARATOR.'images'.DIRECTORY_SEPARATOR.$folder.DIRECTORY_SEPARATOR.$name);

        return HelperController::upload_images($path, $destination, $image, $width, $height);
    }

    public function cropOffer(Request $request)
    {
        $data = $request->image;
        // //data:image/png;base64,
        $image_array_1 = explode(';', $data);

        // base64,
        $image_array_2 = explode(',', $image_array_1[1]);

        // 
        $data = base64_decode($image_array_2[1]);
        $image_name = HelperController::make_slug(Carbon::now());

        $path = public_path('website'.DIRECTORY_SEPARATOR.'images'.DIRECTORY_SEPARATOR.'temporary');
        $destination = public_path('website'.DIRECTORY_SEPARATOR.'images'.DIRECTORY_SEPARATOR.'temporary'.DIRECTORY_SEPARATOR.$image_name.'.jpg');

        if (isset($request->width)) {
            $width = $request->width;
        } else {
            $width = '500';
        }
        if (isset($request->height)) {
            $height = $request->height;
        } else {
            $height = '500';
        }

        HelperController::upload_images($path, $destination, $data, $width, $height);

        return '/website'.DIRECTORY_SEPARATOR.'images'.DIRECTORY_SEPARATOR.'temporary'.DIRECTORY_SEPARATOR.$image_name.'.jpg';
    }

    public static function getImageScale($position)
    {
        switch ($position) {
            case '1':
                return ['width' => '1279', 'height' => '290'];
                break;

            case '2':
                return ['width' => '288', 'height' => '383'];
                break;

            case '3':
                return ['width' => '500', 'height' => '500'];
                break;

            case '4':
                return ['width' => '1279', 'height' => '290'];
                break;

        }
    }
}
