<?php

namespace App\Http\Controllers\Admin;

use App\Http\Controllers\helper\HelperController;
use App\Http\Requests\gallery\CreateGalleryRequest;
use App\Http\Requests\gallery\CreateGalleryTransRequest;
use App\Http\Requests\gallery\UpdateGalleryRequest;
use App\Models\Gallery;
use App\Models\GalleryImage;
use App\Models\GalleryLink;
use App\Models\GalleryTranslation;
use App\Models\GalleryVideo;
use Illuminate\Http\Request;
use Illuminate\Support\Carbon;
use Illuminate\Support\Facades\Session;
use Illuminate\Support\Str;

class GalleryController extends BackendController
{
    public function index()
    {
        if (! in_array('21', Session::get('permissionData'))) {
            return redirect()->back();
        }

        $data['galleries'] = Gallery::all();

        return view('dashboard.admin.galleries.index', $data);
    }

    public function create()
    {
        if (! in_array('22', Session::get('permissionData'))) {
            return redirect()->back();
        }

        return view('dashboard.admin.galleries.create');
    }

    public function addTrans(Request $request)
    {
        if (! in_array('22', Session::get('permissionData'))) {
            return redirect()->back();
        }
        $data['Details'] = Gallery::where('id', $request->gallery_id)->whereHas('GalleryTranslation')->first();
        if ($data['Details'] != null) {
            return redirect('/admin-2023/gallery/edit/'.$request->gallery_id);
        }
        $data['title'] = trans_db('dashboard.CreateNewGalleryTrans');
        $data['id'] = $request->gallery_id;

        return view('dashboard.admin.galleries.trans', $data);
    }

    public function edit(Request $request)
    {
        if (! in_array('23', Session::get('permissionData'))) {
            return redirect()->back();
        }
        $data['Details'] = Gallery::find($request->id);
        if ($data['Details'] == null) {
            return redirect('/admin-2023/gallery/all');
        }
        if ($data['Details']->GalleryTranslation == null) {
            return redirect('/admin-2023/gallery/addTrans/'.$request->id);
        }
        $data['id'] = $request->id;

        return view('dashboard.admin.galleries.edit', $data);
    }

    public function store(CreateGalleryRequest $request)
    {
        $data = self::storeGalleryTranslations($request);
        if ($data == true) {
            alert()->success(trans_db('dashboard.saved'), trans_db('dashboard.congratulation'));

            return redirect('/admin-2023/gallery/all');
        } else {
            alert()->error(trans_db('dashboard.notsaved'), trans_db('dashboard.attention'));

            return redirect('/admin-2023/gallery/create');
        }
    }

    public function addGalleryTrans(CreateGalleryTransRequest $request)
    {
        $test = GalleryTranslation::where('gallery_id', $request->gallery_id)
            ->where('lang_id', app()->getLocale())->first();
        if (isset($test)) {
            alert()->error(trans_db('dashboard.Duplicate_TitleOrLanguage'), trans_db('dashboard.attention'));

            return redirect('/admin-2023/gallery/addTrans/'.$request->gallery_id);
        }

        $data = self::storeGalleryTranslations($request, 'trans');
        if ($data == true) {
            alert()->success(trans_db('dashboard.saved'), trans_db('dashboard.congratulation'));

            return redirect('/admin-2023/gallery/all');
        } else {
            alert()->error(trans_db('dashboard.notsaved'), trans_db('dashboard.attention'));

            return redirect('/admin-2023/gallery/addTrans/'.$request->gallery_id);
        }
    }

    public function update(UpdateGalleryRequest $request)
    {
        $data = self::updateGalleryTranslations($request);

        if ($data == true) {
            alert()->success(trans_db('dashboard.saved'), trans_db('dashboard.congratulation'));
        } else {
            alert()->error(trans_db('dashboard.notsaved'), trans_db('dashboard.attention'));
        }

        return redirect('/admin-2023/gallery/edit/'.$request->gallery_id);
    }

    public static function storeGalleryTranslations(Request $request, $type = null)
    {
        if ($type == null) {
            $CreateGallery = Gallery::create([
                'lang_id' => app()->getLocale(),
                'status' => 1,
                'view_index' => $request->view_index == null || $request->view_index == '' ? 0 : $request->view_index,
            ]);
        }

        if ($type != null) {
            $gallery = Gallery::findOrFail($request->gallery_id);
        }
        $CreateTrans = GalleryTranslation::create([
            'gallery_name' => strip_tags($request->gallery_name),
            'views' => strip_tags($request->views),
            'gallery_location' => strip_tags($request->gallery_location),
            'gallery_date' => strip_tags($request->gallery_date),
            'gallery_id' => isset($CreateGallery) ? $CreateGallery->id : $request->gallery_id,
            'lang_id' => app()->getLocale(),
        ]);

        if ($CreateTrans) {
            self::images(
                $request->image,
                $request->gallery_name,
                isset($CreateGallery) ? $CreateGallery->id : $request->gallery_id,
                $CreateTrans->id,
                app()->getLocale()
            );

            self::videos(
                $request->video,
                $request->gallery_name,
                isset($CreateGallery) ? $CreateGallery->id : $request->gallery_id,
                $CreateTrans->id,
                app()->getLocale()
            );

            self::links(
                $request->link,
                $request->gallery_name,
                isset($CreateGallery) ? $CreateGallery->id : $request->gallery_id,
                $CreateTrans->id,
                app()->getLocale()
            );

            return true;
        }

        return false;
    }

    public static function updateGalleryTranslations(Request $request)
    {
        $Gallery = Gallery::find($request->gallery_id);
        $Gallery->update([
            'lang_id' => app()->getLocale(),
            'view_index' => $request->view_index == null || $request->view_index == '' ? 0 : $request->view_index,
        ]);

        $Trans = GalleryTranslation::where('gallery_id', $request->gallery_id)
            ->where('lang_id', app()->getLocale());
        $Trans->update([
            'gallery_name' => strip_tags($request->gallery_name),
            'gallery_location' => strip_tags($request->gallery_location),
            'gallery_date' => strip_tags($request->gallery_date),
            'gallery_id' => $Gallery->id,
            'lang_id' => app()->getLocale(),
        ]);

        $Trans = GalleryTranslation::where('gallery_id', $request->gallery_id)
            ->where('lang_id', app()->getLocale())->first();

        if (! empty($request->image)) {
            self::images($request->image, $request->gallery_name, $request->gallery_id, $Trans->id, app()->getLocale());

            self::videos(
                $request->video,
                $request->gallery_name,
                $request->gallery_id,
                $Trans->id,
                app()->getLocale()
            );

            self::links(
                $request->link,
                $request->gallery_name,
                $request->gallery_id,
                $Trans->id,
                app()->getLocale()
            );
        }

        return true;
    }

    public static function images($images, $gallery_price, $galleryId, $transId, $lang_id)
    {
        if (! empty($images)) {
            foreach ($images as $image) {
                $imageSlug = HelperController::make_slug($gallery_price.Str::random('8').'_'.str_replace(' ', '', Carbon::today()));
                $image_name = str_replace(' ', '', $imageSlug).'.jpg';

                $relativePath = self::UploadImagesGallery($image, $image_name, 'gallery', '450', '450');

                GalleryImage::create([
                    'image' => $relativePath,
                    'gallery_id' => $galleryId,
                    'translation_id' => $transId,
                    'lang_id' => $lang_id,
                ]);
            }
        }
    }

    public static function videos($videos, $gallery_price, $galleryId, $transId, $lang_id)
    {
        if (! empty($videos)) {
            foreach ($videos as $video) {
                $video_file = HelperController::make_slug($gallery_price).Str::random('8').'_.mp4';

                $path = 'website' . DIRECTORY_SEPARATOR . 'uploads' . DIRECTORY_SEPARATOR . 'videos';
                $fullStoragePath = storage_path('app/public' . DIRECTORY_SEPARATOR . $path);
                if (!file_exists($fullStoragePath)) {
                    mkdir($fullStoragePath, 0755, true);
                }
                $video->move($fullStoragePath, $video_file);
                $relativePath = 'storage/website/uploads/videos/' . $video_file;

                GalleryVideo::create([
                    'video' => $relativePath,
                    'gallery_id' => $galleryId,
                    'translation_id' => $transId,
                    'lang_id' => $lang_id,
                ]);
            }
        }
    }

    public static function links($links, $gallery_price, $galleryId, $transId, $lang_id)
    {
        if (! empty($links)) {
            foreach ($links as $link) {
                GalleryLink::create([
                    'link' => $link,
                    'gallery_id' => $galleryId,
                    'translation_id' => $transId,
                    'lang_id' => $lang_id,
                ]);
            }
        }
    }

    public function delete(Request $request)
    {
        $data = GalleryTranslation::where('gallery_id', $request->id)->first();
        $parentCheck = GalleryTranslation::where('gallery_id', $request->id)->count();
        if ($parentCheck < 2) {
            Gallery::where('id', $request->id)->delete();
        }
        $images = GalleryImage::where('gallery_id', $request->id)->get();
        foreach ($images as $image) {
            if ($image->image) {
                $oldPath = str_replace('storage/', '', $image->image);
                if (\Illuminate\Support\Facades\Storage::disk('public')->exists($oldPath)) {
                    \Illuminate\Support\Facades\Storage::disk('public')->delete($oldPath);
                } elseif (file_exists(public_path('website/images/gallery/' . $image->image))) {
                    unlink(public_path('website/images/gallery/' . $image->image));
                }
            }
        }

        $data->delete();

        alert()->success(trans_db('dashboard.Deleted Successfully..'), trans_db('dashboard.congratulation'));

        return redirect('admin-2023/gallery/all');
    }

    public function delete_image(Request $request)
    {
        $data = GalleryImage::find($request->id);
        if (file_exists(public_path('website'.DIRECTORY_SEPARATOR.'images'.DIRECTORY_SEPARATOR.'gallery'.DIRECTORY_SEPARATOR.$data->image))) {
            unlink('website'.DIRECTORY_SEPARATOR.'images'.DIRECTORY_SEPARATOR.'gallery'.DIRECTORY_SEPARATOR.$data->image);
        }        $data->delete();

        alert()->success(trans_db('dashboard.Deleted Successfully..'), trans_db('dashboard.congratulation'));

        return redirect()->back();
    }

    public function delete_video(Request $request)
    {
        $data = GalleryVideo::find($request->id);
        if (file_exists(public_path('website'.DIRECTORY_SEPARATOR.'uploads'.DIRECTORY_SEPARATOR.'videos'.DIRECTORY_SEPARATOR.$data->video))) {
            unlink('website'.DIRECTORY_SEPARATOR.'uploads'.DIRECTORY_SEPARATOR.'videos'.DIRECTORY_SEPARATOR.$data->video);
        }        $data->delete();

        alert()->success(trans_db('dashboard.Deleted Successfully..'), trans_db('dashboard.congratulation'));

        return redirect()->back();
    }

    public static function getGalleryType($type)
    {
        switch ($type) {
            case $type == 1:
                return trans_db('dashboard.single');

            case $type == 2:
                return trans_db('dashboard.double');

            case $type == 3:
                return trans_db('dashboard.Triple');

            case $type == 4:
                return trans_db('dashboard.Quad');
        }

        return trans_db('dashboard.single');
    }

    public function gallerysOrderUp(Request $request)
    {
        $gallery = Gallery::find($request->id);
        $view_index = $gallery->view_index;

        if ($view_index > 0) {
            $galleries = Gallery::where('view_index', $view_index - 1)->get();
            foreach ($galleries as $galleriess) {
                $galleriess->update(['view_index' => $view_index]);
            }
            $gallery->view_index == null ? $new_index = 1 : $new_index = $view_index - 1;
            $gallery->view_index = $new_index;
            $gallery->save();
        }

        return redirect('/admin-2023/gallery/all');
    }

    public function gallerysOrderDown(Request $request)
    {
        $gallery = Gallery::find($request->id);
        $count = Gallery::count();
        $view_index = $gallery->view_index;

        if ($view_index < $count) {
            $gallery = Gallery::where('view_index', $view_index + 1)->get();
            foreach ($gallery as $Cour) {
                $Cour->update(['view_index' => $view_index]);
            }

            $gallery->view_index = $view_index + 1;
            $gallery->save();
        }

        return redirect('/admin-2023/gallery/all');
    }

    public static function imageUpload(Request $request, $oldImage = null)
    {
        $image_nam = HelperController::make_slug(Str::random('8').'_'.str_replace(' ', '', Carbon::today()));
        $image_name = str_replace(' ', '', $image_nam).'.png';

        if ($request->cropped_image != null && file_exists(public_path($request->cropped_image))) {
            self::UploadImagesGallery(public_path($request->cropped_image), $image_name, 'gallery'.DIRECTORY_SEPARATOR.'small', '181 ', '282');
            self::UploadImagesGallery(public_path($request->cropped_image), $image_name, 'gallery', '334 ', '222');
            unlink(public_path($request->cropped_image));

            return ['image' => $image_name, 'body' => trans_db('dashboard.saved'), 'title' => trans_db('dashboard.congratulation'), 'type' => 'success'];
        }
        if (! empty($request->file('image'))) {
            $ex = $request->file('image')->getClientOriginalExtension();
            if (in_array($ex, ['png', 'jpeg', 'jpg', 'JPG', 'jfif'])) {

                if (isset($oldImage) && $oldImage != null) {
                    $oldPath = str_replace('storage/', '', $oldImage);
                    if (\Illuminate\Support\Facades\Storage::disk('public')->exists($oldPath)) {
                        \Illuminate\Support\Facades\Storage::disk('public')->delete($oldPath);
                    } elseif (file_exists(public_path('website/images/gallery/'.$oldImage))) {
                        unlink(public_path('website/images/gallery/'.$oldImage));
                    }
                }

                self::UploadImagesGallery($request->file('image'), $image_name, 'gallery'.DIRECTORY_SEPARATOR.'small', '181 ', '282');
                $relativePath = self::UploadImagesGallery($request->file('image'), $image_name, 'gallery', '380 ', '290');

                return ['image' => $relativePath, 'body' => trans_db('dashboard.saved'), 'title' => trans_db('dashboard.congratulation'), 'type' => 'success'];
            } else {
                return ['image' => null, 'body' => trans_db('dashboard.notsaved'), 'title' => trans_db('dashboard.attention'), 'type' => 'error'];
            }
        } else {
            return ['image' => null, 'body' => trans_db('dashboard.InValidImage'), 'title' => trans_db('dashboard.attention'), 'type' => 'error'];
        }
    }

    public static function UploadImagesGallery($image, $name, $folder, $width = null, $height = null)
    {
        $path = 'website' . DIRECTORY_SEPARATOR . 'images' . DIRECTORY_SEPARATOR . $folder;
        $fullStoragePath = storage_path('app/public' . DIRECTORY_SEPARATOR . $path);
        $destination = $fullStoragePath . DIRECTORY_SEPARATOR . $name;

        HelperController::upload_images($fullStoragePath, $destination, $image, $width, $height);
        
        return 'storage/website/images/' . $folder . '/' . $name;
    }

    public function cropSlider(Request $request)
    {
        $data = $request->image;
        // //data:image/png;base64,
        $image_array_1 = explode(';', $data);

        // base64,
        $image_array_2 = explode(',', $image_array_1[1]);

        // 
        $data = base64_decode($image_array_2[1]);
        //        return $data;
        $image_name = HelperController::make_slug(Carbon::now());

        $path = public_path('website'.DIRECTORY_SEPARATOR.'images'.DIRECTORY_SEPARATOR.'temporary');
        $destination = public_path('website'.DIRECTORY_SEPARATOR.'images'.DIRECTORY_SEPARATOR.'temporary'.DIRECTORY_SEPARATOR.$image_name.'.jpg');
        HelperController::upload_images($path, $destination, $data, '380', '290');

        return '/website'.DIRECTORY_SEPARATOR.'images'.DIRECTORY_SEPARATOR.'temporary'.DIRECTORY_SEPARATOR.$image_name.'.jpg';
    }

    public function change_status(Request $request)
    {
        $user = Gallery::find($request->gallery_id);
        $user->update([
            'status' => $request->status,
        ]);

        return response()->json(['data' => 'success']);

    }
}
