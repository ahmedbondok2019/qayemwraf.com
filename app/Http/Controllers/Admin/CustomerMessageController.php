...
class CustomerMessageController extends BackendController
{
    public function Index(Request $request)
    {
        ...
        $data['customer_messages'] = CustomerMessage::paginate(15);
    ...
    public function editcontacts(Request $request)
    {
        ...
        $data['customer_messages'] = CustomerMessage::where('id', $request->id)
            ->firstorFail();
    ...
    public function update(Request $request)
    {
        ...
        if ($validator->fails()) {
            if ($validator->fails()) {
                return redirect('/admin-2023/customer_messages/editcontacts/'.$request->id)->withErrors($validator)->withInput();
            }
        }

        $data = CustomerMessage::where('id', $request->id)->first();
        if (isset($data) && $data != '') {
            $data->update([
                'status' => 1,
                'reply' => $request->reply,
            ...
            $Newdata = ['name' => $data->contact_name, 'email' => $data->contact_email, 'message_data' => $data->message, 'reply' => $request->reply];
    ...
    public function delete(Request $request)
    {
        ...
        CustomerMessage::where('id', $request->id)->delete();
